...
def create_log_files(log_all, log_gen, log_pareto_gen, log_pareto):
...
def save_log(gen, log_file, population, fitness_values, fitness_std):
    for c, fit, std in zip(population, fitness_values, fitness_std):
        with open(log_file, 'a', newline='', encoding='utf-8') as f:
            csv.writer(f, delimiter=';').writerow([str(gen), str(c).replace(',', ''), str(fit[0]).replace('.', ','), str(std[0]).replace('.', ','), str(fit[1]).replace('.', ','), str(std[1]).replace('.', ',')])
...
if __name__ == "__main__":
    ...
    create_log_files(log_all, log_gen, log_pareto_gen, log_pareto)

    try:
        population = Initial_Pop()
        pareto_population = []
        tested_chrom = set(); tested_chrom.update(tuple(p) for p in population)
        ...
        save_log(0, log_all, population, fitness_values, fitness_std)
        save_log(0, log_gen, population, fitness_values, fitness_std)
        # for c, fit, std in zip(population, fitness_values, fitness_std):
        ...
        for gen in range(NUM_GENERATIONS):
            ...
            offspring_mut = mutation(population, tested_chrom); tested_chrom.update(tuple(p) for p in offspring_mut)

            offspring_rec = recombination(population, tested_chrom); tested_chrom.update(tuple(p) for p in offspring_rec)
            ...
            save_log(gen+1, log_all, offspring, offspring_fitness_values, offspring_fitness_std)
            ...
            save_log(gen+1, log_pareto_gen, [combined_pop[i] for i in pareto_front_idx], [combined_fit[i] for i in pareto_front_idx], [combined_std[i] for i in pareto_front_idx])
            # for c, fit, std in zip(pareto_population, pareto_fitness, pareto_std):
            #     with open(log_pareto_gen, 'a', newline='', encoding='utf-8') as f:
            #         csv.writer(f, delimiter=';').writerow([gen+1, str(c).replace(',', ''), str(fit[0]).replace('.', ','), str(std[0]).replace('.', ','), str(fit[1]).replace('.', ','), str(std[1]).replace('.', ',')])

            new_population = []; new_fitness = []; new_fitness_std = []
            ...
            save_log(gen+1, log_gen, population, fitness_values, fitness_std)
            # for c, fit, std in zip(population, fitness_values, fitness_std):
            #     with open(log_gen, 'a', newline='', encoding='utf-8') as f:
            #         csv.writer(f, delimiter=';').writerow([gen+1, str(c).replace(',', ''), str(fit[0]).replace('.', ','), str(std[0]).replace('.', ','), str(fit[1]).replace('.', ','), str(std[1]).replace('.', ',')])
        ...
        save_log(NUM_GENERATIONS+1, log_pareto, [population[i] for i in pareto_idx], [fitness_values[i] for i in pareto_idx], [fitness_std[i] for i in pareto_idx])
        # for c, fit, std in zip(pareto_population, pareto_fitness, pareto_std):
